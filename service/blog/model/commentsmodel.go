...
func (m *defaultCommentsModel) Insert(data Comments) (sql.Result, error) {
	...
	ret, err := m.conn.Exec(query, data.CreatedAt, data.UpdateedAt, data.DeletedAt, data.Content, data.Login, data.Name, data.Avatar, data.PostId)
	return ret, err
}
...
func (m *defaultCommentsModel) Update(data Comments) error {
	...
	_, err := m.conn.Exec(query, data.CreatedAt, data.UpdateedAt, data.DeletedAt, data.Content, data.Login, data.Name, data.Avatar, data.PostId, data.Id)
	return err
}
