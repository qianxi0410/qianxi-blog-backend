...
func (m *defaultPostsModel) Count() (int64, error) {
	...
	err := m.conn.QueryRow(&result, query)
...
func (m *defaultPostsModel) Insert(data Posts) (sql.Result, error) {
	...
	ret, err := m.conn.Exec(query, data.Id, data.CreatedAt, data.UpdatedAt, data.DeletedAt, data.Title, data.Description, data.Pre, data.Next, data.Url, data.Path, data.Tags)
	return ret, err
}
...
func (m *defaultPostsModel) Update(data Posts) error {
	...
	_, err := m.conn.Exec(query, data.CreatedAt, data.UpdatedAt, data.DeletedAt, data.Title, data.Description, data.Pre, data.Next, data.Url, data.Path, data.Tags, data.Id)
	return err
}
